...
- `packages/api/src/lib/customDomains.ts` - Cloudflare API client (`cfCreateCustomHostname`/`cfGetCustomHostname`/`cfDeleteCustomHostname`) + **pure helpers** `isValidHostname`, `isReservedHostname` (blocks our app/platform zones - `cubityfir.st`, `workers.dev`, `pages.dev`, `localhost`, `.local`, `.internal` - plus the SaaS-zone apex derived from the CNAME target), `normalizeHostname`, `deriveDnsRecords`, `deriveStatus` (CF hostname status `blocked`/`moved`/`deleted` → `error`, with a human explanation appended by `collectVerificationErrors`), `customDomainsConfigured`, `releaseCustomDomain(env, projectId)` (best-effort CF-hostname deregistration before a site delete - leaves the row to cascade), and `removeCustomDomain(env, projectId)` (standalone unmap: CF deregister best-effort + drop the row itself, returns the hostname; used by the admin remove-domain control). Tests: `customDomains.test.ts`.
- `packages/api/src/routes/customDomains.ts` - `handleCustomDomain` for `/projects/:id/domain` (GET/PUT/DELETE) + `/projects/:id/domain/refresh` (POST). Gates: caller **admin+** (effective role via `resolveRole`) **and** site has `CUSTOM_LINK` flag. **GET lazily re-polls Cloudflare** when the mapping is non-active and the cached row is >60s old (best-effort: a CF failure serves the cache), so the status advances while the settings page polls. **POST /refresh is throttled** (a refresh within 15s of the last update returns the cache without a CF call). PUT validates the hostname (`isValidHostname` + `isReservedHostname`), treats **re-submitting the already-mapped hostname as a refresh** (re-creating it at CF would just conflict), and on a host change **creates the new CF hostname first, then retires the old one** - so a create failure leaves the old mapping serving. A `UNIQUE(hostname)` insert race (another site claimed the host between pre-check and insert) is caught → the just-created CF hostname is released and a friendly 409 returned. CF creates use SSL method `txt`, type `dv`.
...
- `packages/api/src/routes/public.ts` - `GET /public/site-by-host?host=` resolves a mapped host → `{ projectId, vanitySlug, name, homeDocId }` (published sites only; response carries `Cache-Control: max-age=60`). `GET /public/docs/:project/:docId` LEFT JOINs `project_custom_domains` and returns `project.custom_domain` (the hostname when **active**, else null) so the frontend worker can emit canonical URLs.
- `packages/frontend/worker/index.ts` - **host-mode edge serving**: requests on a non-app host (its `isAppHost` copy MUST stay in sync with `lib/siteUrl.ts`) resolve host→site via `site-by-host` (memoized in `caches.default`, 120s) and get (a) per-doc `<title>`/OG/description/canonical injection for `/` (home doc) and `/<docId>` - without this, custom-domain links unfurl as the generic app shell - (b) `robots.txt`, (c) `sitemap.xml` (root + every doc's clean URL, home doc listed only as `/`; built by the exported `buildSitemapXml`). The `/s/:slug/:docId` share-meta path also injects `<link rel="canonical">`: the custom domain when one is active, else the vanity-slug path. Tests: `worker/customHost.test.ts`.
...
- Settings UI: `SiteSettingsPage.tsx` → Site group → "Custom Link & Domain" (the `#custom-domain` card lives inside the `features & 1` block). While the mapping is non-active the card re-fetches `GET /domain` every 30s (which lazy-refreshes server-side); an active domain gets a copy-URL button. Admin app flag label: "Custom Link & Domain" (`ProjectsPage.tsx`).
