...
- **Session-end checkpoint revision**: when the LAST socket closes (before `persist()`), if the tracked editors set is non-empty and the text differs from the latest revision's content, the DO writes a revision via `snapshotDocRevision` (attributed to the first tracked editor, full editor list as `contributors` when >1) and clears the editors set. This guarantees collab sessions that end without an explicit save still leave a history entry and keep FTS/doc-links/AI-summary fresh. Deliberately NOT run from the alarm path (would spam revisions mid-session). Related invariant: `applyDocUpdate`'s "did content change?" baseline is the **latest revision's content** (`latestDocRevisionContent`), NOT the live R2 body - the DO mirrors Y text over the live body, which used to make post-mirror explicit saves silently no-op (dropping revisions + changelogs).
